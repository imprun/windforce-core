...
async function createCdpBrowser(config) {
  ...
  const userDataDir = await mkdtemp(path.join(tmpdir(), "windforce-core-ui-guide-"));
...
async function main() {
  ...
  const verifyScreenshotsDir = transientCapture ? await mkdtemp(path.join(tmpdir(), "windforce-core-ui-guide-verify-")) : "";
