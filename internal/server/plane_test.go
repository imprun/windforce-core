...
func TestServerServesCanonicalHTTPPlanesAndRejectsLegacyAdmission(t *testing.T) {
	handler := New(Config{
	...
	assertHandlerStatus(t, handler, "/api/v1/openapi.json", http.StatusOK)
	...
	assertHandlerMethodStatus(t, handler, http.MethodPost, "/api/v1/w/default/run/example/run", http.StatusNotFound)
	assertHandlerMethodStatus(t, handler, http.MethodPost, "/api/w/default/jobs/run/example/run", http.StatusNotFound)
	assertHandlerMethodStatus(t, handler, http.MethodPost, "/api/w/default/jobs/webhook/example/run", http.StatusNotFound)
