...
func TestDeploymentRequestDeploysPinnedCommit(t *testing.T) {
	tempDir := t.TempDir()
	repoDir := createTestGitSourceRepo(t, tempDir, "repo", "")
	store := bundle.NewLocalStore(filepath.Join(tempDir, "store"))
	fileCatalog := catalog.NewFileCatalog(filepath.Join(tempDir, "catalog.json"))
	...
	createRequestBody, err := json.Marshal(map[string]string{
		"git_source_id": fmt.Sprint(registered.ID),
		"message":       "ready for prod",
	})
	if err != nil {
		t.Fatal(err)
	}
	createReq, err := http.NewRequest(http.MethodPost, server.URL+"/api/w/ws-a/deployment_requests", bytes.NewReader(createRequestBody))
	if err != nil {
		t.Fatal(err)
	}
	createReq.Header.Set("Content-Type", "application/json")
	createReq.Header.Set("X-Windforce-Actor", "developer@example.test")
	...
	if createResp.StatusCode != http.StatusCreated {
		...
		t.Fatalf("create request status = %d, want %d: %s", createResp.StatusCode, http.StatusCreated, body)
	}
	var created struct {
		ID           string `json:"id"`
	...
	if created.ID == "" || created.TargetCommit == "" || created.Status != "pending" {
		t.Fatalf("created request = %#v", created)
	}

	if err := os.WriteFile(filepath.Join(repoDir, "main.ts"), []byte(`export async function main() { return {changed: true} }`), 0o644); err != nil {
		t.Fatal(err)
	}
	runTestGit(t, repoDir, "add", "main.ts")
	runTestGit(t, repoDir, "commit", "-m", "new head")

	deployReq, err := http.NewRequest(http.MethodPost, server.URL+"/api/w/ws-a/deployment_requests/"+created.ID+"/deploy", bytes.NewBufferString(`{"confirm":true,"message":"approved"}`))
	if err != nil {
		t.Fatal(err)
	}
	deployReq.Header.Set("Content-Type", "application/json")
	deployReq.Header.Set("X-Windforce-Actor", "operator@example.test")
	...
	if deployResp.StatusCode != http.StatusOK {
		...
		t.Fatalf("deploy request status = %d, want %d: %s", deployResp.StatusCode, http.StatusOK, body)
	}
	var deployed struct {
		Request struct {
			Status         string  `json:"status"`
	...
	if err := json.NewDecoder(deployResp.Body).Decode(&deployed); err != nil {
		t.Fatal(err)
	}
	if deployed.Request.Status != "deployed" || deployed.Request.ReviewedBy == nil || *deployed.Request.ReviewedBy != "operator@example.test" {
		t.Fatalf("deployed request = %#v", deployed.Request)
	}
	if deployed.SyncResult.Commit != created.TargetCommit || deployed.Request.DeployedCommit == nil || *deployed.Request.DeployedCommit != created.TargetCommit {
		t.Fatalf("deployed commit = sync %q request %v, want pinned %q", deployed.SyncResult.Commit, deployed.Request.DeployedCommit, created.TargetCommit)
	}
	deployment, err := fileCatalog.GetDeploymentForWorkspace(context.Background(), "ws-a", "echo")
	if err != nil {
		t.Fatal(err)
	}
	if deployment.Commit != created.TargetCommit {
		t.Fatalf("catalog commit = %q, want pinned %q", deployment.Commit, created.TargetCommit)
	}
}
