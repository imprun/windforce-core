...
`, workspaceID)
		...
		queries := []string{
			...
			`DELETE FROM runtime_config_audit WHERE workspace_id=$1`,
			`DELETE FROM runtime_config_operation WHERE workspace_id=$1`,
			`DELETE FROM runtime_variable WHERE workspace_id=$1`,
			`DELETE FROM runtime_resource WHERE workspace_id=$1`,
