...
func rateBucketKey(workspaceID string, limit KeyedRateLimitPin) string {
	return contract.NormalizeWorkspace(workspaceID) + "\x00" + limit.Scope + "\x00" + limit.PolicyID + "\x00" + limit.KeyDigest + "\x00" + strconv.FormatInt(int64(limit.WindowSeconds), 10)
}
...
func rateLimitsReached(snapshot *Snapshot, candidate Job, now time.Time) bool {
	for _, limit := range candidate.Payload.ExecutionLimits.Rate {
		...
		if ok && bucket.WindowStart.Equal(start) && bucket.Consumed >= limit.MaxAttempts {
			return true
		}
	}
	return false
...
func demandRateLimitsReached(candidate Job, usage map[string]int) bool {
	workspaceID := normalizedJobWorkspace("", candidate)
	for _, limit := range candidate.Payload.ExecutionLimits.Rate {
		if !validKeyedRatePin(limit) {
			return true
		}
		if usage[rateBucketKey(workspaceID, limit)] >= int(limit.MaxAttempts) {
			return true
		}
	}
	return false
