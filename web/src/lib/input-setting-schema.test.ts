...
describe("inputSettingDefinitions", () => {
  test("combines request fields and operator-only settings", () => {
    const definitions = inputSettingDefinitions(
      {
        type: "object",
        properties: { OWNERNAME: { type: "string", description: "Vehicle owner" } },
      },
      {
        type: "object",
        properties: {
...
describe("validateInputSettingValue", () => {
  ...
  test("accepts a documented value", () => {
    expect(
      validateInputSettingValue(definition, { gap: "gov24-plus", eul: "gov24" }),
    ).toBeUndefined();
  });

  test("explains missing, unknown, and invalid values", () => {
    expect(validateInputSettingValue(definition, { gap: "gov24-plus" })).toBe(
      "ROUTE.eul is required.",
    );
    expect(validateInputSettingValue(definition, { gap: "other", eul: "gov24" })).toContain(
      "must be one of",
    );
    expect(
      validateInputSettingValue(definition, { gap: "gov24-plus", eul: "gov24", extra: true }),
    ).toBe("ROUTE.extra is not an allowed field.");
  });
});
