...
describe("actionDisplayName", () => {
  test("removes implementation notes and input suffixes", () => {
    expect(actionDisplayName("정산내역조회 입력값 (4MDCPCM M1 — runtime notes)")).toBe(
      "정산내역조회",
    );
    expect(actionDisplayName("Coupang Eats login session issue input")).toBe(
      "Coupang Eats login session issue",
    );
  });
