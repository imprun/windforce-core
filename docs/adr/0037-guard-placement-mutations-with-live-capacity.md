...
Core derives each candidate selector with the same worker tag and required-label functions used to pin Jobs. An engine-owned execution-profile label remains required even when an operator supplies a required-label override. Eligible capacity consists of live Workers in `active` status whose canonical registry selector matches the candidate tag, labels, and execution profile. The precondition and registered claim path call the same selector helper defined by [ADR 0038](0038-bind-registered-worker-claims-to-immutable-lease-identity.md), and the State Store rejects a registered Worker claim whose request differs from that advertisement. Managed Workers also require an active, unexpired credential scoped to the workspace and a WorkerGroup that accepts new work. Registered static Workers use the same authoritative selector without a managed group fence. Unregistered static compatibility claims remain possible but have no registry record and therefore cannot count as placement capacity.
