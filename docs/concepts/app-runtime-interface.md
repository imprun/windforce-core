...
The built-in Database Secret backend creates no external candidate object. A side-effecting backend may implement the optional runtime-candidate collector contract: Core marks current sealed Variable references, the backend enumerates only its dedicated candidate namespace, and an old unreferenced object is removed with a versioned conditional delete. Exact retries touch the same deterministic object, while tombstone and revoke retain the current reference and audited purge makes it eligible for eventual physical collection. See [ADR 0051](../adr/0051-collect-orphaned-runtime-secret-candidates.md).
