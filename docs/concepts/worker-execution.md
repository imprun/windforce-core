...
A worker may bind an optional loopback capability gateway. The worker discovers ready providers before advertising its configured placement labels. For a claimed Job whose effective labels intersect those gateway labels, the worker opens one Job-scoped run and sends the actual Run ID, Job ID, and Attempt in authenticated request headers. It then injects only the gateway's opaque reference, short-lived token, loopback URL, and ready provider IDs through reserved private runtime metadata. The worker-wide token and Core execution identity never enter the App process through that binding. The Job token is included in log and result masking, and the run is closed on success, failure, interruption, or cancellation. A failed run-open request may contribute bounded ASCII lower-case snake-case `phase` and `reason` values plus a `retryable` hint to the existing `RuntimeBindingError`; Core discards the raw gateway body and does not retry automatically. Syntax validation cannot identify a secret or resource identifier disguised as a valid code, so the worker-token-authenticated operator gateway MUST emit only registered, stable, non-secret codes, and any provider adapter MUST map upstream errors through an explicit allowlist. Core does not proxy provider calls or binary artifacts; see [ADR 0034](../adr/0034-bind-worker-local-capability-gateways.md) and [ADR 0052](../adr/0052-preserve-safe-runtime-binding-failure-details.md).
...
A failure before App launch remains an engine error value rather than an Action result. Runtime binding preserves the existing `RuntimeBindingError` name, message, `ExitCode=-1`, and Job error while adding the registered `phase` and `reason` codes plus the `retryable` field defined by ADR 0052. The hint does not change Job state, Attempt accounting, or retry behavior. Local state, PostgreSQL state, remote Worker completion, the Control Plane Job result, and the canonical Invocation result carry the same JSON value.
